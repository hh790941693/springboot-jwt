	...
	<script type="text/javascript">	
		...
		function drawClock(){
			...
			for (var i=15;i<75;i++){
				...
				ctx.translate(X,Y);
				ctx.beginPath();
				var jiaodu = i*6*Math.PI/180;

				var xTmp=Math.round(R*Math.cos(jiaodu));
				var yTmp=Math.round(R*Math.sin(jiaodu));
				
				var fontXTmp = Math.round((R-25)*Math.cos(jiaodu));
				var fontYTmp = Math.round((R-25)*Math.sin(jiaodu));
				
				var x=xTmp,y=-yTmp;
				var fontX=fontXTmp,fontY=-fontYTmp;

				var radius = 5,color="blue";
				if (i%5==0){
					radius = 10;
					color = "red";
					...
					ctx.fillStyle="white";
					ctx.fillText(hourShow,fontX,fontY);
					hourShow--;
				}
				ctx.arc(x,y,radius,0,2*Math.PI,false);
				//ctx.arc(fontX,fontY,radius,0,2*Math.PI,false);
				//ctx.closePath();
				ctx.fillStyle=color;
				ctx.fill();
				ctx.restore();
		...
		var minutesHand = new drawObject("minute",X,Y,X,0,140,"white",3);
