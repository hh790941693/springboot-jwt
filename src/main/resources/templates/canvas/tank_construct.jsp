...
<html>
	...
	<script type="text/javascript">	
		...
		// 游戏模式
		var SINGLE_PLAYER = 1,MULTI_PLAYER = 2, CONSTRUCTION = 3;
		
		// 玩家模式 1:单人模式  2:双人模式
		var GAME_MODE = SINGLE_PLAYER;
		
		// 鼠标左键单击次数
		var LEFT_MOUSE_CLICK_COUNT = -1;
		...
		// 从json文件中读取所有关卡信息
		var MISSION_CFG = readJsonByAjax("mission.json");
			...
			if (key_state){
				...
				if (!IS_GAME_START && !IS_GAME_PAUSE && !IS_GAME_OVER){
                    if (keyCode==38){
                        ...
                        if (GAME_MODE != CONSTRUCTION){
                            // 开始游戏
                            startGame();
                        }else{
                            IS_CONSTRUCT = true;
							IS_SHOW_GRID = true;
							loadMission("construct");
                            $("#canvas").mousedown(rightMousedownListener);
                            $("#canvas").mousedown(leftMousedownListener);
                        }
					}
				}
				// 如果游戏运行中 点击p键暂停游戏  
        ...
        function leftMousedownListener(event){
            ...
            if (key_state){
				LEFT_MOUSE_CLICK_COUNT++;
				if (LEFT_MOUSE_CLICK_COUNT > 5){
					LEFT_MOUSE_CLICK_COUNT = 0;
				}
				
				var box = canvas.getBoundingClientRect();
				var mouseX = parseInt(event.clientX-box.left);
				var mouseY = parseInt(event.clientY-box.top);
				
				var row = parseInt(mouseY/DIV_WIDTH)+1;
				var col = parseInt(mouseX/DIV_HEIGHT)+1;
				
				if (LEFT_MOUSE_CLICK_COUNT == 5){
					var temp = MISSION_CFG["construct"].getObjKeysAsArr();
					for (var i=0;i<MISSION_CFG["construct"].getObjKeysAsArr().length;i++){
						var blockName = MISSION_CFG["construct"].getObjKeysAsArr()[i];
						if (isHasArr(MISSION_CFG["construct"][blockName], new Array([row,row,col,col]))){
							var index = getExistArrIndx(MISSION_CFG["construct"][blockName],new Array([row,row,col,col]));
							if (typeof(index) != "undefined"){
								MISSION_CFG["construct"][blockName].splice(index,1);
							}
						}
					}
				}else{
					var block_name = BLOCK_OBJ.getObjKeysAsArr()[LEFT_MOUSE_CLICK_COUNT];
					// 绘图前,先清除该位置的障碍物
					var temp = MISSION_CFG["construct"].getObjKeysAsArr();
					for (var i=0;i<MISSION_CFG["construct"].getObjKeysAsArr().length;i++){
						var blockName = MISSION_CFG["construct"].getObjKeysAsArr()[i];
					...
					MISSION_CFG["construct"][block_name] = MISSION_CFG["construct"][block_name].concat(new Array([row,row,col,col]));
				}
				loadMission("construct");
            }
        }
		...
		function readJsonByAjax(jsonFileName){
			...
			$.ajax({
				...
				error : function(result) {
					console.log("parse " + jsonFileName + " failed");
				}
			});
		...
		function drawGameIndexPage(){
			...
			}else if (GAME_MODE == CONSTRUCTION){
                ctx.drawImage(SPRITE_IMAGE,srcPicInfo.x,srcPicInfo.y,srcPicInfo.w,srcPicInfo.h,200,427,30,30);
            }
			// 显示关卡数
			ctx.beginPath();
		...
		function ainimate(){
			...
			if (!IS_CONSTRUCT && !IS_GAME_START && !IS_GAME_PAUSE && !IS_GAME_OVER){
				// 绘制游戏首页文字及动画
				drawGameIndexPage();
			...
			if (IS_GAME_START){
				...
				if (G_CYCLE %300 == 0){
                    console.log("关卡数:" + MISSION + "   障碍物:"+STICK_ARR.length + "   敌军:" + ENERMY_ARR.length + "   子弹:" + BOMB_ARR.length + "    追踪炮弹:" + TRACK_BOMB_ARR.length);
                }
