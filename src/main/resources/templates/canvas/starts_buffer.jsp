...
<html>
	...
	<script type="text/javascript">	
		...
		var buffCanvasCtxArr=new Array();
		function createCancas(width,height){
			var canvasTmp = document.createElement("canvas");
			canvasTmp.width = width;
			canvasTmp.height = height;
			var canvasCtxTmp = canvasTmp.getContext("2d");
			buffCanvasCtxArr.push(canvasCtxTmp);
			
			return canvasCtxTmp;
		}
		
		var plantBufferCtx = createCancas(canvas.width,canvas.height);
		
		// 画轨道
		function drawTrack(){
			var bufferCtx = createCancas(canvas.width,canvas.height);
			for (var i=0;i<8;i++){
				bufferCtx.beginPath();
				bufferCtx.arc(CENTER_X,CENTER_Y,(i+1)*SETP_DIS,0,2*Math.PI,false);
				bufferCtx.closePath();
				bufferCtx.strokeStyle = "#fff";
				bufferCtx.setLineDash([1,2]);
				bufferCtx.stroke();
			}
		...
		*/
		function Star(name,parentName,radius,distanceFromCenter,cycle,color,angle){
			...
			this.draw = function(){
				this.bufferCtx.save();
				this.bufferCtx.translate(this.centerX,this.centerY);  // 移动画布的原点至(500,500)
				...
				this.bufferCtx.arc(this.x,this.y,this.radius,0,2*Math.PI,false);
				this.bufferCtx.fillStyle = this.color;
				this.bufferCtx.fill();//填充颜色
				...
				if (starLevel == STAR_LEVEL.son){
					if (hasSon(this.name)){
						this.bufferCtx.beginPath();
						this.bufferCtx.setLineDash([1,2]);
						this.bufferCtx.arc(this.x,this.y,25,0,2*Math.PI,false);
						this.bufferCtx.strokeStyle = this.color;
						this.bufferCtx.stroke();//填充颜色
					}
				}

				if (starLevel == STAR_LEVEL.grandson){
					this.bufferCtx.font = "bold 6px Arial";	
				}else{
					this.bufferCtx.font = "bold 10px Arial";
				}
				this.bufferCtx.fillStyle="white";
				if (this.name == "太阳"){
					this.bufferCtx.fillText(this.name,this.x-11,this.y+5);
				}else{
					this.bufferCtx.fillText(this.name,this.x-5,this.y-13);
				}
		...
		for (let i=0;i<40;i++){
			let r = randomNumber(1,3);
			let R = r + randomNumber(1,3);
			let x = randomNumber(10,canvas.width-10);
			let y = randomNumber(10,canvas.height-10);
			let opcity = Math.random();
			let tmpStar = new StarFive(r,R,x,y,1,"white","white",opcity);
			starFiveArr.push(tmpStar);
		}
		
		function move(){
			ctx.clearRect(0,0,1000,900);
			for (let i=0;i<buffCanvasCtxArr.length;i++){
				buffCanvasCtxArr[i].clearRect(0,0,570,570);
			}
			drawTrack();
			
			//for (let i=0;i<buffCanvasCtxArr.length;i++){
			//	ctx.drawImage(buffCanvasCtxArr[i].canvas,0,0);
			//}
