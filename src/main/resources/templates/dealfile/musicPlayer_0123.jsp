...
<html>
				...
				<div style="height:75px;">
					...
					<div style="float:right;border:1px solid bulue;margin-top:12px;width:740px;">
						<div style="float:left;margin-left:10px;height:40px;">
							...
							<button id="btnPlay" style='height:40px;width:74px;' class='btn btn-success' onclick="playMusic()">播放</button>
	...
	<script type="text/javascript">
		...
		function playMusic(){
			...
			var inputArr = $(parent.inputObjArr);
			var allTrArr= $(parent.trObjArr);
			var trObj = $(parent.trObjArr).eq(1);
			...
			//$("table input").removeAttr("checked");
			$(inputArr).removeAttr("checked");
			$(inputObj).prop("checked",true);
			//$("table tr").removeClass("selectRowClass");
			$(allTrArr).removeClass("selectRowClass");
			$(trObj).addClass("selectRowClass");
			//console.log("start to play:" + url);
			
			//$("#player").attr("src",url);
			$(parent.playerObj).attr("src",url);

			//$("#songName").html(songname);
			$(parent.songNameObj).html(songname);
			var btnText = $(parent.palyBtnObj).text();
			if (btnText == "播放"){
				btnText = "暂停";
				//player.play();
				$(parent.playerObj)[0].play();
			}else{
				btnText = "播放";
				$(parent.playerObj)[0].pause();
				//player.pause();
			}
			$(parent.palyBtnObj).text(btnText);
		}

		// 下一首
		function nextMusic(){
			console.log("start to play next music......");
			var inputArr = $("table td input");
		...
		// 上一首
		function preMusic(){
			console.log("start to play pre music......");
			var inputArr = $("table td input");
		...
		var mediaTmp = $("#player")[0];
		var eventTester = function(e){
			mediaTmp.addEventListener(e,function(){
	            if (e == "ended"){
	            	if (playMode == 0){
	            		// 单曲循环
	            		playMusic();
