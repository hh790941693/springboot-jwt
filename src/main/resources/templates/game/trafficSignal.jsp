...
<html>
	<head>
		...
		<style type="text/css">
			...
			.manClass1,.manClass2,.manClass3,.manClass4,.manClass5,.manClass6,.manClass7,.manClass8,.manClass9,.manClass10{
				background-image: url("<%=path%>/img/man.jpg");
				background-repeat: no-repeat;
				background-size:25px 25px;
			}
	...
	<body>
		<div id="gameMainDiv" style="width:840px;height:554px;border:2px solid green;margin:1px auto;padding:10px;">
			<div id="borderDiv" style="width:710px;height:550px;float:left;margin-left:1px;">
			</div>
...
//		];
			...
			for (var c=1;c<=COL_MAX;c++){
				...
				if (r == 1){
					...
					if (c%COL_MAX==startCol+2){
						$("#"+divId).text("北");
						$("#"+divId).css({"color":"blue","font-size":"20px","text-align":"center","font-weight":"bold"});
	...
	function setInputShowTimer(direction,colorType,initTime){
		...
		$("#borderDiv ."+SIGNAL_TIMER_PREFIX+direction).css({"color":color,"font-size":"20px","text-align":"center"});
		var num = initTime-1;
