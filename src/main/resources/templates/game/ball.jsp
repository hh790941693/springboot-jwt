...
<html>
	...
	<body>
		<div id="gameMainDiv" style="width:760px;height:590px;border:2px solid green;margin:5px auto;padding:10px;">
			...
			<div id="controlDiv" style="width:250px;height:590px;border:2px solid grey;float:right">
				<div id="buttonDiv">
					<div id="modeDiv" style="padding-left:10px">
						...
						<input type="radio" value="4" name="mode">变态
					</div>
					<br/>
	...
	<script type="text/javascript">	
		...
		function randomSpecifiedNumber(){
			var paraLength = arguments.length;
			var numIndex = randomNumber(0, paraLength-1);
			
			return arguments[numIndex];
		}
					...
					if ($("#"+ballGps).hasClass("bottomWallClass") && $("#"+ballGps).hasClass("rightWallClass")){
						...
						if ($("#"+ballGps).hasClass("rightWallClass")){
							...
							if (leftWoodGps== ballGps+1+COL_MAX){
								// 如果当前球在左侧木头的左上角位置    左上运动
								nextGps = ballDirection(ballGps, LEFT, TOP);								
							}else if (leftWoodGps== ballGps+COL_MAX){
								nextGps = ballDirection(ballGps, LEFT, TOP);
							}else if (leftWoodGps== ballGps+1){
								// 如果当前球在左侧木头的左边     左下运动
								nextGps = ballDirection(ballGps, LEFT, BOTTOM);
						...
						if (!$("#"+ballCurPos).hasClass("leftWallClass")){
							...
							while(true){
								...
								if (ballNext>=startNo && ballNext<endNo){
									...
									var left1 = ballNext-1;
									var left2 = ballNext-2;
									var right1 = ballNext+1;
									var right2 = ballNext+2;
									//console.log("球落地点位置:"+ballNext);
									if (ballNext == startNo){
										repaitWood(randomSpecifiedNumber(right1,right2));
										//console.log("木板中心位置:"+right1+","+right2);
									}else if (ballNext == startNo+1){
										repaitWood(randomSpecifiedNumber(ballNext,right1,right2));
										//console.log("木板中心位置:"+""+ballNext+","+right1+","+right2);
									}else if (ballNext == endNo-1){
										repaitWood(randomSpecifiedNumber(left1, ballNext));
										//console.log("木板中心位置:"+left1+","+ballNext);
									}else{
										repaitWood(randomSpecifiedNumber(left1,ballNext,right1,right2));
										//console.log("木板中心位置:"+left1+","+ballNext+","+right1+","+right2);
									}
						...
						if (!$("#"+ballCurPos).hasClass("rightWallClass")){
							...
							while(true){
								...
								if (ballNext>startNo && ballNext<=endNo){
									...
									//repaitWood(ballNext-1);
									if (ballNext == endNo){
										repaitWood(randomSpecifiedNumber(left2,left1));
									}else if (ballNext == endNo-1){
										repaitWood(randomSpecifiedNumber(left2,left1,ballNext));
									}else if (ballNext == startNo+1){
										repaitWood(randomSpecifiedNumber(ballNext, right1));
									}else{
										repaitWood(randomSpecifiedNumber(left2,left1,ballNext,right1));
									}
									break;
								}
								ballNext = ballNext+1+COL_MAX;
